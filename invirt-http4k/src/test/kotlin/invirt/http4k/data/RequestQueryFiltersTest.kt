...
class RequestQueryFiltersTest : StringSpec({
    ...
    "query params to filter - OR" {
        ...
        filter(Request(Method.GET, "/test?type=person")) shouldBe CompoundFilter.and(
            CompoundFilter.or("entity-type" eq "person")
        )

        filter(Request(Method.GET, "/test?type=person&type=company")) shouldBe CompoundFilter.and(
            CompoundFilter.or("entity-type" eq "person", "entity-type" eq "company")
        )

        filter(Request(Method.GET, "/test?type=person&min-age=18")) shouldBe CompoundFilter.and(
            CompoundFilter.or("entity-type" eq "person"),
            "minAge".gte(18)
        )

        filter(Request(Method.GET, "/test?type=person&min-age=18&min-age=25")) shouldBe CompoundFilter.and(
            CompoundFilter.or("entity-type" eq "person"),
            "minAge".gte(18)
        )
    ...
    "query params to filter - AND" {
        val filter = queryValuesFilter(CompoundFilter.Operator.AND) {
            Query.multi.optional("type").or { "entity-type".eq(it) }
            Query.int().optional("min-age") filter { "minAge".gte(it) }
            Query.enum<MaritalStatus>().multi.optional("marital-status").or { "maritalStatus".eq(it) }
            Query.optional("signed-up") filter { value ->
