...
class ErrorResponseTest : StringSpec({
    ...
    "errorResponse - response status" {
        ...
        routes("/test" bind Method.GET to { errorResponse(it, errors, "errorResponse-utils-test") })
            .invoke(Request(Method.GET, "/test"))
            .status shouldBe Status.UNPROCESSABLE_ENTITY

        routes("/test" bind Method.GET to { errorResponse(it, errors, "errorResponse-utils-test", null, Status.BAD_REQUEST) })
            .invoke(Request(Method.GET, "/test"))
            .status shouldBe Status.BAD_REQUEST
    }

    "errorResponse - error pairs" {
        routes("/test" bind Method.GET to { errorResponse(it, "errorResponse-utils-test", "test" to uuid7()) })
            .invoke(Request(Method.GET, "/test"))
            .status shouldBe Status.UNPROCESSABLE_ENTITY
    }
