...
fun resourceAsInput(classpathLocation: String): InputStream =
    Thread.currentThread().contextClassLoader.getResourceAsStream(classpathLocation)!!

fun resourceAsString(classpathLocation: String, charset: Charset = Charsets.UTF_8): String = resourceAsInput(classpathLocation).use {
...
fun resourceAsStrings(classpathLocation: String, charset: Charset = Charsets.UTF_8): List<String> =
    resourceAsString(classpathLocation, charset).split("\n").map {
        it.cleanWhitespace()
    }
