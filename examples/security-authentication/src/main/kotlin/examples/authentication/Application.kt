...
import invirt.pebble.functions.request
...
class Application {
    ...
    fun start() {
        ...
        val pebbleExtensions = listOf(
            pebbleFunctions(
                pebbleFunction("currentUser") { context.request.principal }
            )
        )

        Invirt.configure(pebble = InvirtPebbleConfig(extensions = pebbleExtensions))

        val authService = AuthenticationService()

        val appHandler = AuthenticationFilter(authService)
